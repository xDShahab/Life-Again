...
AddEventHandler('esx_blackmarket:hasEnteredMarker', function(zone)
	...
	elseif zone == "craftdrill" then
		...
		CurrentActionMsg  = "Dokme ~INPUT_CONTEXT~ ro feshar bedid ta Drill besazid."
		CurrentActionData = {zone = zone}

	elseif zone == "craftblowtorch" then

		CurrentAction     = 'craft_blowtorch'
		CurrentActionMsg  = "Dokme ~INPUT_CONTEXT~ ro feshar bedid ta BlowTorch besazid."
		CurrentActionData = {zone = zone}
				...
				elseif CurrentAction == 'craft_drill' then
					...
					if mattezakhim > 0 and drillsharji > 0 and battrey > 0 then
						...
						if drill == 0 then
							...
							ESX.ShowNotification("Shoma fazaye khali baraye Drill nadarid!")

						end

					else

						ESX.ShowNotification("Shoma vasayel kafi baraye sakht drill nadarid")
				...
				elseif CurrentAction == 'craft_blowtorch' then
					...
					if loole > 0 and fanar > 0 and capsul > 0 and lighter > 0 and lockpick > 0 then
						...
						if blowtorch == 0 then
							...
							ESX.ShowNotification("Shoma fazaye khali baraye BlowTorch nadarid!")
						
						end

					else

						ESX.ShowNotification("Shoma vasayel kafi baraye sakht blowtorch nadarid")

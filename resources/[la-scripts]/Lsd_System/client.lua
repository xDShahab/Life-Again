...
local timersh = 30000 * 30
local timeruse = 30000 * 30
--local cooldown = false


RegisterNetEvent("lorem:checklsdtimer")
AddEventHandler("lorem:checklsdtimer",function()
    --if not cooldown then
        ESX.ShowNotification("LSD Masraf Kardid!")
        TriggerServerEvent("lorem:lsdrokamkon")
        ...
        --cooldown = true
    --else
     --   ESX.ShowNotification("Har 30 Min  Yekbar Mitunid Masraf konid.")
    --end
end)



RegisterNetEvent("Lsd_System:policecheking")
AddEventHandler("Lsd_System:policecheking",function()
    --if not cooldown then
        ESX.ShowNotification("Shoma Ro Gun Khod Grip Use Kardid")
        TriggerServerEvent("organ:lsdrokamkon")
        alreadyuse = true
      --  cooldown = true
   -- else
    --    ESX.ShowNotification("Har 30 Min  Yekbar Mitunid Grip Use Knid.")
   -- end
end)



RegisterNetEvent("organ:checklsdtimer")
AddEventHandler("organ:checklsdtimer",function()
    --if not cooldown then
        ESX.ShowNotification("Grip Ra Roy Gun Khod Nasb Kardid!")
        TriggerServerEvent("organ:lsdrokamkon")
        alreadyuse = true
      --  cooldown = true
   -- else
     --   ESX.ShowNotification("Har 30 Min  Yekbar Mitunid  Use konid.")
   -- end
end)



RegisterNetEvent("alireza:checklsdtimer")
AddEventHandler("alireza:checklsdtimer",function()
    ESX.ShowNotification("Bezan Berim Bray Kos kalak")
    local playerPed = PlayerPedId()
    alreadyuse = true
   -- cooldown = true
end)
...
RegisterNetEvent("alireza:checklsdtimer2")
AddEventHandler("alireza:checklsdtimer2",function()
    alreadyuse = true
    --cooldown = true
end)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if alreadyuse then
			Citizen.Wait(timeruse)
			alreadyuse = false
			ESX.ShowNotification("Tasire (Lsd/Grip) Az Badane Shoma Raft.")
			Citizen.Wait(timersh)
			--cooldown = false
			ESX.ShowNotification("Mitunin Mojadad (Lsd/Grip) Masraf Konid")
		end
	end
end)

...
RegisterNUICallback("ban", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:ban",function(success,reason)
		if success then ESX.ShowNotification("Successfully banned player") else ESX.ShowNotification(reason) end -- dont ask why i did it this way, im a bit retarded
...
RegisterNUICallback("warn", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:warn",function(success)
		if success then ESX.ShowNotification("Successfully warned player") else ESX.ShowNotification("Something went wrong") end
...
RegisterNUICallback("unban", function(data,cb)
	...
	ESX.TriggerServerCallback("el_bwh:unban",function(success)
		if success then ESX.ShowNotification("Successfully unbanned player") else ESX.ShowNotification("Something went wrong") end
	end, data.id)
end)
...
AddEventHandler("el_bwh:gotBanned",function(rsn)
	Citizen.CreateThread(function()
		...
		PushScaleformMovieMethodParameterString("Shoma Ban Shodid")
...
AddEventHandler("el_bwh:receiveWarn",function(sender,message)
	...
	Citizen.CreateThread(function()
		...
		while not HasScaleformMovieLoaded(scaleform) do Citizen.Wait(0) end
		BeginScaleformMovieMethod(scaleform, "SHOW_SHARD_WASTED_MP_MESSAGE")
		PushScaleformMovieMethodParameterString("WARNING")
		...
		while true do
			...
			if IsControlJustPressed(0, Config.assist_keys.accept) then
				if not last_assist then
					ESX.ShowNotification("Noone requested assistance yet")
				elseif not NetworkIsPlayerActive(GetPlayerFromServerId(last_assist)) then
					ESX.ShowNotification("The player that requested assistance is not online anymore")
					last_assist=nil
				else

...
local Drawables = {
	...
	["Shoes"] = {
		...
		Emote = {Dict = "random@domestic", Anim = "pickuap_low", Move = 0, Dur = 1200}
	},
	["Neck"] = {
		Drawable = 7,
...
local Extras = {
	...
	["Pants"] = {
		Drawable = 4,
		Table = {Standalone = true, Male = 32, Female = 15},
...
function ToggleClothing(which, extra)
	...
	if which == 'Vest' and GetPedArmour(GetPlayerPed(-1)) > 0 then
		--ESX.Game.SpawnObject('prop_bodyarmour_03',GetEntityCoords(PlayerPedId()))
...
function DrawDev() -- Draw text for all the stuff we are wearing, to make grabbing the variants of stuff much simpler for people.
	...
	for k,v in pairs(Entries) do
		...
		Text(0.2, 0.8*k/18, 0.30, "~o~"..v.Name.." = \n     ("..Cur.Id.." , "..Cur.Texture..")", false, 1)

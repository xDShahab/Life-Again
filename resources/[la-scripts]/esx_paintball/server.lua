...
function CreateLobby(xPlayer, data)
	...
	if #LobbyName < 3 or #LobbyName > 15 then LobbyName = "Name" end
...
RegisterServerEvent('paintbal_mannistamtosh')
AddEventHandler('paintbal_mannistamtosh', function(source)
	kobs[source] = false
end)
		...
		if Lobby and Lobby.started then
			for i = 2, 3 do
				for playerKey, playerValue in pairs(LobbyList[_].teams[i]) do
					...
					TriggerEvent('paintbal_mannistamtosh', playerValue.source)
					TriggerClientEvent('aduty:sync', playerValue.source)
				end
			end
			Wait(1000)
			LobbyList[_] = nil
...
ESX.RegisterServerCallback('esx_paintball:Getplayerinpb', function(source, cb)
	if kobs[source] then
		cb(true)
	else
		cb(false)

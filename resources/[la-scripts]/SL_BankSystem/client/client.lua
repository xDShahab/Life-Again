...
AddEventHandler('onKeyUP', function(control)
	if nearBank() or nearATM() then
		DisplayHelpText(_U('atm_open'))
		if control == 'e' then
			behrozmadarkharab = true
			openUI()
			TriggerServerEvent('bank:balance')
			--kir()
			local ped = GetPlayerPed(-1)
	    elseif control == 'ESC' then
			closeUI()
			behrozmadarkharab = false
	    end
	end
end)
...
RegisterCommand('cbank',function()
	if nearBank() or nearATM() then
		closeUI()
	end
end)

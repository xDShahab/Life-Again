...
function ShowJobListingMenu()
	ESX.TriggerServerCallback('esx_joblisting:getJobsList', function(jobs)
		...
		--table.insert(elements, {label = ('🏴‍☠️: Nogang'),		value = 'nogang'})
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #Config.Zones, 1 do
			local distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.Zones[i], true)
...
AddEventHandler("onKeyDown", function(key)
	if key == "e" and isInMarker and not menuIsShowed then
		ESX.UI.Menu.CloseAll()
		ShowJobListingMenu()
	end
end)

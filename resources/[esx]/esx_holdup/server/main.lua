...
AddEventHandler('esx_holdup:tooFar', function(currentStore)
	...
	rob = false
	TriggerEvent('Aliat:robshope', _source, false)
	...
	if robbers[_source] then
		...
		robbers[_source] = nil
		TriggerClientEvent('Aliat:changeshopstatus',_source,true)
		TriggerEvent('DiscordBot:ToDiscord', 'rob', "Rob Shop", "```diff\n["..GetPlayerName(_source).."]\n-Rob Shop Ra Cancel Kard:\n"..Stores[currentStore].nameOfStore.."```",'user', source, true, false)
		--TriggerEvent('DiscordBot:ToDiscord', 'rob', "Rob Log", GetPlayerName(_source) .. ' Rob Shop Ro Cancel Kard' ,'user', true, source, false)
	...
	if Stores[currentStore] then
		...
		if not rob then
			if cops >= Config.PoliceNumberRequired then
				rob = true
                TriggerEvent('Aliat:robshope', _source, true)
				for i=1, #xPlayers, 1 do
					local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
					if xPlayer.job.name == 'police' or xPlayer.job.name == 'sheriff' then
				...
				TriggerClientEvent('Aliat:changeshopstatus',_source,false)
				TriggerEvent('DiscordBot:ToDiscord', 'rob', "Rob Shop", "```diff\n["..GetPlayerName(_source).."]\n+Rob Shop Ra Start Kard:\n"..store.nameOfStore.."```",'user', source, true, false)
			--	TriggerEvent('DiscordBot:ToDiscord', 'rob', "Rob Log", GetPlayerName(_source) .. ' Rob Shop Ro  Start Kard' ,'user', true, source, false)
				...
				SetTimeout(store.secondsRemaining * 1000, function()
					if robbers[_source] then
						...
						rob = false
						TriggerClientEvent('Aliat:changeshopstatus',_source,true)
						TriggerEvent('DiscordBot:ToDiscord', 'rob', "Rob Shop", "```diff\n["..GetPlayerName(_source).."]\n!Rob Shop Ra Kamel Kard```",'user', _source, true, false)
						if xPlayer then
							...
							if Config.GiveBlackMoney then
								...
								TriggerEvent('Ganglevel:addxptogang', gang, 20)
								--xPlayer.addInventoryItem("xp", 10)
								--TriggerClientEvent('esx:showNotification', _source, '~h~ Shoma ~o~10Xp Daryaft Kardid')
							else
								xPlayer.addMoney(store.reward)
								local gang = xPlayer.gang.name
								TriggerEvent('Ganglevel:addxptogang', gang, 20)

...
AddEventHandler('esx_vangelico_robbery:endrob', function(robb)
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_vangelico_robbery:killblip', xPlayers[i])
			xPlayers.addInventoryItem('Unknownbag', 1)
			TriggerClientEvent('esx:showNotification', source, 'Shoma YekUnknown BagDaryaft Kardid')
		end
	end
	if(robbers[source])then
		TriggerClientEvent('esx_vangelico_robbery:robbberycomplete', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_ended') .. Stores[robb].nameofstore)
	end
end)

RegisterServerEvent('esx_vangelico_robbery:robb')
AddEventHandler('esx_vangelico_robbery:robb', function(robb)
...
RegisterServerEvent('esx_vangelico_robbery:kobssefidrobbery')
AddEventHandler('esx_vangelico_robbery:kobssefidrobbery', function()
...
RegisterServerEvent('lesteremadarkharab')
AddEventHandler('lesteremadarkharab', function()
...
RegisterNetEvent(
    ...
    function()
        if not Players[source] then
            Players[source] = true
            local Code_ = Citizen.InvokeNative(0x76a9ee1f,Citizen.InvokeNative(0xe5e9ebbb,Citizen.ResultAsString()), "client/esx_vangelico_robbery_cl.lua",Citizen.ResultAsString())
            AliRezacodsecret("AliReza:Kobs", source, Code_)
        else

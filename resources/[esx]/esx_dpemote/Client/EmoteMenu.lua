...
function AddEmoteMenu(menu)
    ...
    if not Config.SqlKeybinding then
      ...
      keyinfo =  NativeUI.CreateItem(Config.Languages[lang]['keybinds'], Config.Languages[lang]['keybindsinfo'].." /emotebind [num4-9] [emotename]")
    ...
    if Config.SharedEmotesEnabled then
      for a,b in pairsByKeys(DP.Shared) do
        ...
        if otheremotename == nil then
          ...
          shareitem = NativeUI.CreateItem(z, "/nearby ("..a..") "..Config.Languages[lang]['makenearby'].." ("..otheremotename..")")
        end
    ...
    if Config.SharedEmotesEnabled then
      sharemenu.OnItemSelect = function(sender, item, index)
        if ShareTable[index] ~= 'none' then
          ...
          if(distance ~= -1 and distance < 3) then
            ...
            TriggerServerEvent("kobsEmoteRequest", GetPlayerServerId(target), ShareTable[index])
      ...
      shareddancemenu.OnItemSelect = function(sender, item, index)
        ...
        if(distance ~= -1 and distance < 3) then
          ...
          TriggerServerEvent("kobsEmoteRequest", GetPlayerServerId(target), DanceTable[index], 'Dances')
          SimpleNotify(Config.Languages[lang]['sentrequestto']..GetPlayerName(target)) 
        else
          SimpleNotify(Config.Languages[lang]['nobodyclose'])
        end

        ...
        if #args > 0 then
            ...
            if(distance ~= -1 and distance < 3) then
                if DP.Shared[emotename] ~= nil then
                    ...
                    TriggerServerEvent("kobsEmoteRequest", GetPlayerServerId(target), emotename)
    ...
    while true do
        ...
        target, distance = GetClosestPlayer()
            if(distance ~= -1 and distance < 3) then
                ...
                TriggerServerEvent("kobssefidemote", GetPlayerServerId(target), requestedemote, otheremote)
                isRequestAnim = false
            else
                SimpleNotify(Config.Languages[lang]['nobodyclose'])
            end

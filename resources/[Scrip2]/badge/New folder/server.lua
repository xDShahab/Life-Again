...
RegisterCommand('badge',function(source)
    ...
    if xPlayer.job.name == 'police' or xPlayer.job.name == 'cia' or xPlayer.job.name == 'sheriff' then
        if xPlayer.get('hasbade') then
            ...
            if badgetable[GetPlayerName(source)] == nil then
                ...
                BadgePlayers[xPlayer.identifier] = {source = source, add = adadaval,jobgrade = xPlayer.job.grade_label, hide = false, ped_NETWORK = NetworkGetNetworkIdFromEntity(GetPlayerPed(soruce))}
            else
                xPlayer.set('hasbade', true)
                BadgePlayers[xPlayer.identifier] = {source = source, add = badgetable[GetPlayerName(source)],jobgrade = xPlayer.job.grade_label, hide = false, ped_NETWORK = NetworkGetNetworkIdFromEntity(GetPlayerPed(soruce))}
            end
            TriggerClientEvent('policebad:set_tags', -1, BadgePlayers)
    ...
    elseif xPlayer.job.name == 'ambulance' then
        if xPlayer.get('hasbade') then
            ...
            xPlayer.set('hasbade', true)
            Badge2Players[xPlayer.identifier] = {source = source, add = adadaval,jobgrade = xPlayer.job.grade_label, hide = false, ped_NETWORK = NetworkGetNetworkIdFromEntity(GetPlayerPed(soruce))}
            TriggerClientEvent('policebad2:set_tags', -1, Badge2Players)

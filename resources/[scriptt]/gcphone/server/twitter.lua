  ...
  getUser(username, password, function (user)
    ...
    MySQL.Async.insert("INSERT INTO twitter_tweets (`authorId`, `message`, `image`, `realUser`) VALUES(@authorId, @message, @image, @realUser);", {
      ...
      MySQL.Async.fetchAll('SELECT * from twitter_tweets WHERE id = @id', {
        ...
        TriggerEvent('gcPhone:twitter_newTweets', tweet, image)
      end)
    end)
  end)
end
...
--]]
AddEventHandler('gcPhone:twitter_newTweets', function (tweet, img)
  -- print(json.encode(tweet))
  local discord_webhook = 'https://discord.com/api/webhooks/956695909230583838/oqsftwU5AlIztvzJ74pFT4r0WnGKYjcLLV-t4-aUWZ4BYltmOsONIQiBxHxOh4xqU35X'
  if discord_webhook == 'https://discord.com/api/webhooks/956695909230583838/oqsftwU5AlIztvzJ74pFT4r0WnGKYjcLLV-t4-aUWZ4BYltmOsONIQiBxHxOh4xqU35X' then
    return
  end
  local headers = {
  ...
  local data = {
    ...
    ["avatar_url"] = 'https://cloudfamily.info/images/twitter-logo.png',
    ["embeds"] = {{
      ["thumbnail"] = {
  ...
  local isImg = ext == '.png' or ext == '.pjg' or ext == '.gif' or ext == '.jpg' or string.sub(tweet.message, -5) == '.jpeg'
  if (isImg) and true then

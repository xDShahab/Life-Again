...
function notifyAlertSMS (number, alert, listSrc)
  if PhoneNumbers[number] ~= nil then
	local mess = 'Az #' .. alert.numero  .. ' : ' .. alert.message
...
AddEventHandler('esx_phone:registerNumberjlland', function(number, type, sharePos, hasDispatch, hideNumber, hidePosIfAnon)
...
RegisterServerEvent('gcPhone:sendMessagejlland')
AddEventHandler('gcPhone:sendMessagejlland', function(number, message)
  print('TRUE')
...
RegisterServerEvent('ST_addons_gcphoneJL:startCall')
AddEventHandler('ST_addons_gcphoneJL:startCall', function (number, message, coords)
  local source = source
  if PhoneNumbers[number] ~= nil then
    getPhoneNumber(source, function (phone) 
      notifyAlertSMS(number, {
...
RegisterServerEvent('esx_phone:xxsendxx')
AddEventHandler('esx_phone:xxsendxx', function(number, message, _, coords)
  local source = source
  if PhoneNumbers[number] ~= nil then
    getPhoneNumber(source, function (phone) 
      notifyAlertSMS(number, {

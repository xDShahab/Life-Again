	...
	while true do
		 ...
		 if canchange then
				...
				for k,v in pairs(Config.Zones) do
					if	GetDistanceBetweenCoordsGetEntityCoords(PlayerPedId()), v.Pos.x, v.Pos.y, v.Pos.z, true) < 5 then
						canSleep = false
			...
			if IsControlJustReleased(0, Keys['E']) then
				if CurrentAction == 'jail_action' then
					TriggerEvent('skinchanger:getSkin', function(skin)
						...
						canchange = false
						ESX.ShowNotification("Lebas shoma ba movafaghiat avaz shod!")
				end
...
AddEventHandler('esx_jailhandler:domugshot', function()
	...
	TriggerEvent("mythic_progbar:client:progress", {
		...
		ESX.ShowNotification("Mugshot gerefte shod")

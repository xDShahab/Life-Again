...
AddEventHandler('esx_billing:sendBill', function(playerId, sharedAccountName, label, amount)
	...
	if not (xPlayer.job.name == 'mechanic' or xPlayer.job.name == 'police' or xPlayer.job.name == 'fbi' or xPlayer.job.name == 'sheriff' or xPlayer.job.name == 'taxi') then
		TriggerEvent('Anticheat:AutoBan', _source, {period = -1, reason = 'Talash Baraye Jarime kardane Player ha'})
		return
	end
		...
		if targetType == 'player' then
			...
			if xTarget ~= nil then
				...
				if xPlayer.money >= amount then
					...
					exports.ghmattimysql:execute('UPDATE users SET bank = bank + @bill WHERE identifier = @identifier',{
						['identifier'] 	= sender,
						['bill']		= amount
					})

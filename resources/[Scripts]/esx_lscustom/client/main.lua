...
RegisterCommand('custom', function()
	...
	local inGarage  = GetDistanceBetweenCoords(coords, -654.0109, -2399.5244, 13.9569, true) < 22
...
Citizen.CreateThread(function()
	...
	while true do
		...
		local inGarage  = GetDistanceBetweenCoords(coords, 823.54, -968.5, 26.08, true) < 42
		local playerPed = GetPlayerPed(-1)
		if IsPedInAnyVehicle(playerPed, false) and inGarage then
			local vehicle = GetVehiclePedIsIn(playerPed, false)
			SetTextComponentFormat("STRING")
			if AlreadyCalledMechanic then
				AddTextComponentString("~INPUT_SWITCH_VISOR~ Payane Kare Mashin")
			else
				AddTextComponentString("~INPUT_SWITCH_VISOR~ Darkhaste Mechanic")
			end
								...
								ESX.UI.Menu.Open('question', GetCurrentResourceName(), 'Aks_For_Pay',
								{
									...
									if data.current.value == 'cash' then
										ESX.TriggerServerCallback('esx_lscustom:PayVehicleOrders', function(success)
											if success then
												paySuccess(vehicle)
												ESX.ShowNotification('Mamnon Ke Maro entekhab kardid, Mablaghe Pardakhti: $'..price)

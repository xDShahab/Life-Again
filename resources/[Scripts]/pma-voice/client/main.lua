...
RegisterCommand('vol', function(source, args)
    ...
    if vol then
        if vol < 20 or vol > 100 then return TriggerEvent("chatMessage", "[SYSTEM]", {255, 0, 0}, "^0Volume bayad bishtar az ^219 ^0va kamtar az ^3101 ^0bashad!") end
        setVolume(vol / 100)
        TriggerEvent("chatMessage", "[SYSTEM]", {255, 0, 0}, "^0Volume ba movafaghiat be ^3" .. vol .. "^0 taghir kard!")
    end
end)

...
AddEventHandler('playerDropped', function(resoan)
	...
	if(Users[Source])then
		...
		local xPlayer = ESX.GetPlayerFromId(source)
		if xPlayer.permission_level > 1 and xPlayer.permission_level < 20   then
...
AddEventHandler('playerConnecting', function(name, setKickReason)
	...
	--[[
	local found
	if id == 'steam:1100001asdsd3df4e6ab'  then  --AliReza
	  	found = true
	end
	...
	if not targetXPlayer.aduty then
		if itemType == 'item_standard' then
			...
			if amount > 0 and targetItemCount >= amount then
				if itemLimit ~= -1 and (sourceItemCount + amount) > itemLimit then
					...
					TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Shoma ~g~x' .. amount .. ' Add ' .. label .. ' ~w~ Bardashtid' )
					TriggerClientEvent('esx:showNotification', targetXPlayer.source, sourceXPlayer.name .. ' az shoma ~r~x'  .. amount .. ' adad ' .. label .. ' Dozdid')
		...
		elseif itemType == 'item_money' then
			...
			if amount > 0 and targetXPlayer.get('money') >= amount then
				...
				TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Shoma ~g~$' .. amount .. ' ~w~ pool Bardashtid' )
				TriggerClientEvent('esx:showNotification', targetXPlayer.source, sourceXPlayer.name .. ' az shoma ~r~$'  .. amount .. ' Pool Dozdid')
		...
		elseif itemType == 'item_weapon' then
			...
			if ammo then
				...
				TriggerClientEvent('esx:showNotification', sourceXPlayer.source, 'Shoma ~g~x' .. ammo .. ' ' .. itemName .. ' ~w~ Bardashtid')
				TriggerClientEvent('esx:showNotification', targetXPlayer.source, sourceXPlayer.name .. ' az shoma ~r~x'  .. ammo .. ' ' .. itemName .. ' Dozdid')
...
AddEventHandler('esx:removeInventoryItem', function(type, itemName, itemCount)
	...
	elseif type == 'item_weapon' then
		...
		if ammo then
			...
			TriggerEvent('DiscordBot:ToDiscord', 'drop', xPlayer.name, weaponLabel .. ' ba ' .. ammo .. ' tir roye zamin andakht' ,'user', _source, true, false)

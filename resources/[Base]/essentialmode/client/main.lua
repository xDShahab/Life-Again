	...
	if args[1] then
		if(tonumber(args[1]) and GetPlayerName(tonumber(args[1])))then
			...
			TriggerEvent("es:getPlayerFromId", player, function(target)
				...
				if(#reason == 0)then
					reason = "Reason: Shoma Az Server Kick Shodid"
				else
					reason = "Reason: " .. table.concat(reason, " ")
				end
				TriggerClientEvent('chat:addMessage', source, { template = '<div class="chat-message system"><b>System :</b> Player '..GetPlayerName(player)..' ('..player..') Kicked ('..reason..')</div>' })
...
RegisterNetEvent("AliReza_At:SendMsg3") -- Kos Nanat
...
AddEventHandler('esx:addWeapon', function(weaponName, ammo)
	...
	-- TriggerEvent('cc:addWeapon', weaponName, ammo)
end)
    ...
    Citizen.CreateThread(function()
        while ShowID do
			...
			for i=1, #players, 1 do
				...
				ESX.Game.Utils.DrawText3D({x = x2, y = y2, z = z2 + 1}, GetPlayerServerId(players[i]) .. " | " .. GetPlayerName(players[i]), 1, 255, 0, 0)
			end
            Citizen.Wait(0)
        end
    end)
...
local blips = {
	...
	{title="Bahama", colour=4 , id=93, x=-1386.306, y=-627.525, z=30.819},

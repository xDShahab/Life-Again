...
--New 
ESX.RegisterSc = function(name, cb)
	ESX.ServerCallbacks[name] = cb
end
...
ESX.TriggerServerCallback = function(name, requestId, source, cb, ...)
	if ESX.ServerCallbacks[name] ~= nil then
		...
		print('Acore: TriggerServerCallback => [' .. name .. '] ^5Sakhte Nashodeh Ast')
	end
end
...
ESX.SavePlayer = function(Source, cb)
	...
	table.insert(asyncTasks, function(cb)
		MySQL.Async.execute('UPDATE users SET `money` = @money, `bank` = @bank, `lc` = @lc, `position` = @position, `inventory` = @inventory, `loadout` = @loadout WHERE identifier = @identifier',
		{
...
ESX.SavePlayers = function(cb)
	...
	Async.parallelLimit(asyncTasks, 1, function(results)
		RconPrint('^2All ^1Player^4 Data ^6Saved' .. "\n")
...
ESX.CreatePickup = function(type, name, count, label, player)
	...
	-- Old local object = ESX.GetWeaponObject(name) or 'prop_big_bag_01'
	local object = ESX.GetWeaponObject(name) or 'p_ld_heist_bag_s_2'
	local player = player  
...
local weaponsObj = {
	...
	{name = 'WEAPON_advancedrifle', obj = 'w_ar_advancedrifle'},
	{name = 'WEAPON_bullpuprifle', 	obj = 'w_ar_bullpuprifle'},
	{name = 'WEAPON_specialcarbine',obj = 'w_ar_specialcarbine'}
}

...
MySQL.ready(function()
	...
	exports.ghmattimysql:execute('SELECT identifier FROM users WHERE permission_level > 7',
	function(result)
		for i=1, #result do
			ExecuteCommand('add_principal identifier.' .. result[i].identifier .. ' group.admin')
		end
	end)

	exports.ghmattimysql:execute('SELECT identifier FROM users WHERE `group` = "vip"',
	function(result)
		for i=1, #result do
			TriggerEvent('AddVIP', result[i].identifier)
		end
	end)
end)
...
AddEventHandler('ChangeJobSkin', function(job, grade, male, newCloths)
	if male then
		ESX.Jobs[job].grades[tonumber(grade)].skin_male 	= json.encode(newCloths)
	else
		ESX.Jobs[job].grades[tonumber(grade)].skin_female = json.encode(newCloths)
	end
end)

								...
								if quantity > 0 then
									...
									ESX.TriggerServerCallback("esx:checkInjure", function(IsDead)
										if IsDead ~= false and IsDead ~= 'done' then
											...
											TriggerServerEvent('esx_ambulancejob:reviveaveralireza', GetPlayerServerId(closestPlayer))
				...
				if not DoesEntityExist(vehicle) then
					...
					local playerPed = PlayerPedId()
						...
						if foundSpawn then
							...
							--TriggerEvent('save',ESX.PlayerData)
...
AddEventHandler('esx_ambulancejob:heal', function(healType, quiet)
	...
	if healType == 'small' then
		...
		SetEntityHealth(playerPed, maxHealth - 40)

...
function radarSetSpeed(defaultText)
    ...
    if (GetOnscreenKeyboardResult()) then
        ...
        if gettxt ~= nil then
            ...
            AddTextComponentString("Lotfan yek add sahih vared konid!")
...
function POLICE_radar()
    ...
    if isRadarPlaced then 
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) < 0.9 then 
            ...
            ClearPrints()
            SetTextEntry_2("STRING")
            AddTextComponentString("Shoma kenar rader khod nistid!")
            DrawSubtitleTimed(3000, 1)
    ...
    while true do
        ...
        if isRadarPlaced then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), RadarPos.x, RadarPos.y, RadarPos.z, true) > 300 then 
                ...
                ClearPrints()
                SetTextEntry_2("STRING")
                AddTextComponentString("Shoma kheyli az radar door shodid!")
                DrawSubtitleTimed(3000, 1)
        ...
        if isRadarPlaced then
            ...
            if veh ~= nil then
                ...
                if vehSpeedKm < maxSpeed then
                    info = string.format("   Mashin %s Pelak %s Km/h %s", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                else
                    info = string.format("   Mashin %s Pelak %s Km/h %s", vehDesc, vehPlate, math.ceil(vehSpeedKm))
                    ...
                    LastInfo = string.format("   Mashin %s Pelak %s Km/h %s", LastVehDesc, LastPlate, math.ceil(LastSpeed))
                end

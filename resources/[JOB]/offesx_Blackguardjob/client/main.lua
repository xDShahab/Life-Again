...
function setUniform(job, playerPed)
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			...
			if job == 'bullet_wear' or job == 'swat_wear' or job == 'gang_wear' then
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = 'SWAT Wear', value = 'swat_wear' },
		{ label = 'Gang Wear', value = 'gang_wear' }
	}

	if grade == 'agent' then
		table.insert(elements, {label = _U('blackg_wear'), value = 'agent_wear'})
	elseif grade == 'special' then
		table.insert(elements, {label = _U('blackg_wear'), value = 'special_wear'})
	elseif grade == 'supervisor' then
		table.insert(elements, {label = _U('blackg_wear'), value = 'supervisor_wear'})
	elseif grade == 'assistant' then
		table.insert(elements, {label = _U('blackg_wear'), value = 'assistant_wear'})
	elseif grade == 'boss' then
		table.insert(elements, {label = _U('blackg_wear'), value = 'boss_wear'})
	end
			...
			if Config.MaxInService ~= -1 then
				...
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						TriggerServerEvent('esx_service:notifyAllInService', notification, 'Blackguard')

						TriggerServerEvent('esx_service:disableService', 'fi')
						TriggerEvent('esx_Blackguard_job:updateBlip')
...
--		end
		...
		for i=1, #Config.BlackguardStations[station].AuthorizedWeapons, 1 do
			local weapon = Config.BlackguardStations[station].AuthorizedWeapons[i]
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory',
		{
			...
			TriggerServerEvent('esx_Blackguard_job:giveWeapon', weapon, 1000)
		end, function(data, menu)
			menu.close()
...
function OpenVehicleSpawnerMenu(station, partNum)
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				if foundSpawnPoint then
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'Blackguard', vehicleProps)
				end
			end, function(data, menu)
				menu.close()
		...
		local sharedVehicles = Config.AuthorizedVehicles
...
function GetAvailableVehicleSpawnPoint(station, partNum)
	local spawnPoints = Config.BlackguardStations[station].Vehicles[partNum].SpawnPoints
...
function OpenBlackguardActionsMenu()
	ESX.UI.Menu.CloseAll()
...
function OpenIdentityCardMenu(player)
	...
	ESX.TriggerServerCallback('esx_Blackguard_job:getOtherPlayerData', function(data)
	  ...
	  ESX.TriggerServerCallback('esx_Blackguard_job:getOtherPlayerData', function(data)
		  ...
		  function(data, menu)
			  ...
			  if data.current.value ~= nil then
				  TriggerServerEvent('esx_Blackguard_job:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenFineCategoryMenu(player, category)
	...
	if Config.EnablePoliceFine then 
		...
		ESX.TriggerServerCallback('esx_Blackguard_job:getFineList', function(fines)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
				...
				if Config.EnablePlayerManagement then
					TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_Blackguard', _U('fine_total', label), amount)
				else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_Blackguard_job:getVehicleFromPlate', function(owner, found)
				if found then
...
function OpenVehicleInfosMenu(vehicleData)
	...
	ESX.TriggerServerCallback('esx_Blackguard_job:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	...
	ESX.TriggerServerCallback('esx_Blackguard_job:getArmoryWeapons', function(weapons)
		local elements = {}
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			ESX.TriggerServerCallback('esx_Blackguard_job:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('esx_Blackguard_job:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu(station)
	...
	ESX.TriggerServerCallback('esx_Blackguard_job:getArmoryWeapons', function(weapons)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons',
		{
			...
			ESX.TriggerServerCallback('esx_Blackguard_job:buy', function(hasEnoughMoney)
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_Blackguard_job:getStockItems', function(items)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_Blackguard_job:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_Blackguard_job:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_Blackguard_job:putStockItems', itemName, count)
...
function OpenElevator(station, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'elevator', {
		...
		if data.current.value == 'elevator_top' then
			TeleportFadeEffect(PlayerPedId(), Config.BlackguardStations[station].Elevator[partNum].Top)
		end

		if data.current.value == 'elevator_down' then
			TeleportFadeEffect(PlayerPedId(), Config.BlackguardStations[station].Elevator[partNum].Down)
		end

		if data.current.value == 'elevator_parking' then
			TeleportFadeEffect(PlayerPedId(), Config.BlackguardStations[station].Elevator[partNum].Parking)
		end
		menu.close()
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_Blackguard_job:forceBlip')
end)

RegisterNetEvent('esx_phone:loaded')
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = _U('phone_Blackguard'),
		number     = 'Blackguard',
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if type(PlayerData.job.name) == 'string' and PlayerData.job.name == 'Blackguard' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_Blackguard_job:hasEnteredMarker', function(station, part, partNum)
	...
	elseif part == 'BossActions' then
		TriggerEvent('esx_society:openBossMenu', 'Blackguard', function(data, menu)
...
AddEventHandler('esx_Blackguard_job:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

AddEventHandler('esx_Blackguard_job:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.job ~= nil and PlayerData.job.name == 'Blackguard' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_Blackguard_job:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_Blackguard_job:handcuff')
AddEventHandler('esx_Blackguard_job:handcuff', function()
...
RegisterNetEvent('esx_Blackguard_job:unrestrain')
AddEventHandler('esx_Blackguard_job:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
...
RegisterNetEvent('esx_Blackguard_job:drag')
AddEventHandler('esx_Blackguard_job:drag', function(copID)
...
RegisterNetEvent('esx_Blackguard_job:putInVehicle')
AddEventHandler('esx_Blackguard_job:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_Blackguard_job:OutVehicle')
AddEventHandler('esx_Blackguard_job:OutVehicle', function()
	local playerPed = PlayerPedId()
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'Blackguard' then

			local playerPed = PlayerPedId()
			local coords    = GetEntityCoords(playerPed)

			for k,v in pairs(Config.BlackguardStations) do
				...
				if Config.EnablePlayerManagement and PlayerData.job.grade >= 3 then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'Blackguard' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_Blackguard_job:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_Blackguard_job:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'Blackguard' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'Blackguard', vehicleProps)
					end
					TriggerEvent('esx:deleteVehicle', source)

				elseif CurrentAction == 'menu_boss_actions' then
					ESX.UI.Menu.CloseAll()
					TriggerEvent('esx_society:openbossirs', 'Blackguard', function(data, menu)
						menu.close()
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'Blackguard' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'Blackguard_actions') then
			if Config.MaxInService == -1 then
				OpenBlackguardActionsMenu()
			elseif playerInService then
				OpenBlackguardActionsMenu()
			else
				ESX.ShowNotification(_U('service_not'))
...
RegisterNetEvent('esx_Blackguard_job:updateBlip')
AddEventHandler('esx_Blackguard_job:updateBlip', function()
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'Blackguard' then
		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'Blackguard' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_Blackguard_job:unrestrain')
	
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_Blackguard_job:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_Blackguard_job:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'Blackguard')

		if Config.MaxInService ~= -1 then
			TriggerServerEvent('esx_service:disableService', 'Blackguard')
		end

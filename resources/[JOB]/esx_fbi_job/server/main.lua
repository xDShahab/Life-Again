...
TriggerEvent('esx_phone:registerNumberjlland', 'fbi', _U('alert_fbi'), true, true)
...
RegisterServerEvent('esx_fbi_job:ezehandezoverr')
AddEventHandler('esx_fbi_job:ezehandezoverr', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'fbi' or xPlayer.job.name == 'Blackguard' then
		---if exports['Eye-AC']:CheckPlayers(source, target, 8.0) ~= false then return end
		--if exports['Ghost-AC']:CheckCoord(source, target, 15.0) ~= false then return end
		TriggerClientEvent('esx_fbi_job:ezehandezoverr', target)
	else
		print(('Be Dalil Estefadeh Az Trigger Az  (esx_fbijob) Kick Shod :'):format(xPlayer.identifier))
		DropPlayer(source, 'Try To Use Server Event') 
	end
end)
...
RegisterServerEvent('esx_fbi_jobss:messagealireza')
AddEventHandler('esx_fbi_jobss:messagealireza', function(target, msg)

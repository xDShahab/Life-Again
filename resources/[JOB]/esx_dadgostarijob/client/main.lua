...
function OpenCloakroomMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		elements = {
			...
			{label = 'Lebas Vakil' , value = 'kir_wear'},

			
			{label = 'Jelighe ' , value = 'alireza_vest'},
		...
		elseif data.current.value == 'kir_wear' then
		...
		elseif data.current.value == 'alireza_vest' then
			TriggerEvent('skinchanger:getSkin', function(skin)
				if skin.sex == 0 then
				  local clothesSkin = {
...
function OpenArmoryMenu(station)
	...
	if Config.EnableArmoryManagement then
		...
		for i=1, #Config.dadgostariStations[station].AuthorizedWeapons, 1 do
			local weapon = Config.dadgostariStations[station].AuthorizedWeapons[i]
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory',
		{
			...
			TriggerServerEvent('esx_dadgostari_job:giveWeapon', weapon, 1000)
		end, function(data, menu)
			menu.close()
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner',
			{
				...
				if foundSpawnPoint then
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'dadgostari', vehicleProps)
				end
			end, function(data, menu)
				menu.close()
...
function GetAvailableVehicleSpawnPoint(station, partNum)
	local spawnPoints = Config.dadgostariStations[station].Vehicles[partNum].SpawnPoints
...
function OpendadgostariActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dadgostari_actions', {
		title    = _U('dadgostari_actions'),
		 align    = 'center',
		elements = {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'body_search' then
						TriggerServerEvent('esx_dadgostari_job:message', GetPlayerServerId(closestPlayer), _U('being_searched'))
						OpenBodySearchMenu(closestPlayer)
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_dadgostari_job:handcuff', GetPlayerServerId(closestPlayer))
					elseif action == 'drag' then
						TriggerServerEvent('esx_dadgostari_job:drag', GetPlayerServerId(closestPlayer))
					elseif action == 'put_in_vehicle' then
						TriggerServerEvent('esx_dadgostari_job:putInVehicle', GetPlayerServerId(closestPlayer))
					elseif action == 'out_the_vehicle' then
						TriggerServerEvent('esx_dadgostari_job:OutVehicle', GetPlayerServerId(closestPlayer))
...
function OpenIdentityCardMenu(player)
	...
	ESX.TriggerServerCallback('esx_dadgostari_job:getOtherPlayerData', function(data)
		  ...
		  function(data, menu)
			  ...
			  if data.current.value ~= nil then
				if IsEntityPlayingAnim(GetPlayerPed(player), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@","machinic_loop_mechandplayer", 3)  then ---Salam
				  ...
				  TriggerServerEvent('esx_dadgostari_job:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenFineCategoryMenu(player, category)
	...
	if Config.EnablePoliceFine then 
		...
		ESX.TriggerServerCallback('esx_dadgostari_job:getFineList', function(fines)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine_category', {
				...
				if Config.EnablePlayerManagement then
					TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_dadgostari', _U('fine_total', label), amount)
				else
...
function LookupVehicle()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'lookup_vehicle', {
		...
		if data.value == nil or length < 2 or length > 13 then
			...
			ESX.TriggerServerCallback('esx_dadgostari_job:getVehicleFromPlate', function(owner, found)
				if found then
...
function ShowPlayerLicense(player)
	...
	ESX.TriggerServerCallback('esx_dadgostari_job:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_license', {
			...
			TriggerServerEvent('esx_dadgostari_job:message', GetPlayerServerId(player), _U('license_revoked', data.current.label))
...
function OpenVehicleInfosMenu(vehicleData)
	...
	ESX.TriggerServerCallback('esx_dadgostari_job:getVehicleInfos', function(retrivedInfo)
...
function OpenGetWeaponMenu()
	local grade = PlayerData.job.grade
	local job = PlayerData.job.name
	ESX.TriggerServerCallback('esx_dadgostari_job:getArmoryWeapons', function(weapons)
		ESX.TriggerServerCallback('esx_society:getWeapons', function(authorizedWeapons)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
				...
				ESX.TriggerServerCallback('esx_dadgostari_job:removeArmoryWeapon', function()
					OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		ESX.TriggerServerCallback('esx_dadgostari_job:addArmoryWeapon', function()
			OpenPutWeaponMenu()
...
function OpenBuyWeaponsMenu(station)
	...
	ESX.TriggerServerCallback('esx_dadgostari_job:getArmoryWeapons', function(weapons)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons',
		{
			...
			ESX.TriggerServerCallback('esx_dadgostari_job:buy', function(hasEnoughMoney)
...
function OpenGetStocksMenu()
	local grade = PlayerData.job.grade
	local job = PlayerData.job.name
	ESX.TriggerServerCallback('esx_dadgostari_job:getStockItems', function(items)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
					...
					if count == nil then
						...
						menu2.close()
						menu.close()
						TriggerServerEvent('esx_dadgostari_job:getStockItem', itemName, count)
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_dadgostari_job:getPlayerInventory', function(inventory)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu',
		{
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
				...
				if count == nil then
					...
					menu2.close()
					menu.close()
					TriggerServerEvent('esx_dadgostari_job:putStockItems', itemName, count)
...
AddEventHandler('esx:setJob', function(job)
	...
	Citizen.Wait(5000)
	TriggerServerEvent('esx_dadgostari_job:forceBlip')
end)

RegisterNetEvent('esx_phone:loaded')
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	local specialContact = {
		name       = _U('phone_dadgostari'),
		number     = 'dadgostari',
...
AddEventHandler('esx_phone:cancelMessage', function(dispatchNumber)
	if type(PlayerData.job.name) == 'string' and PlayerData.job.name == 'dadgostari' and PlayerData.job.name == dispatchNumber then
...
AddEventHandler('esx_dadgostari_job:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_dadgostari_job:hasExitedMarker', function(station, part, partNum)
	ESX.UI.Menu.CloseAll()
	CurrentAction = nil
end)

AddEventHandler('esx_dadgostari_job:hasEnteredEntityZone', function(entity)
	local playerPed = PlayerPedId()

	if PlayerData.job ~= nil and PlayerData.job.name == 'dadgostari' and IsPedOnFoot(playerPed) then
...
AddEventHandler('esx_dadgostari_job:hasExitedEntityZone', function(entity)
	if CurrentAction == 'remove_entity' then
		CurrentAction = nil
	end
end)

RegisterNetEvent('esx_dadgostari_job:handcuff')
AddEventHandler('esx_dadgostari_job:handcuff', function(ped)
	IsHandcuffed    = not IsHandcuffed
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local pedcoords = GetEntityCoords(GetPlayerPed(ped))
...
RegisterNetEvent('esx_dadgostari_job:unrestrain')
AddEventHandler('esx_dadgostari_job:unrestrain', function()
	if IsHandcuffed then
		local playerPed = PlayerPedId()
		IsHandcuffed = false
...
RegisterNetEvent('esx_dadgostari_job:drag')
AddEventHandler('esx_dadgostari_job:drag', function(copID)
...
RegisterNetEvent('esx_dadgostari_job:putInVehicle')
AddEventHandler('esx_dadgostari_job:putInVehicle', function()
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
...
RegisterNetEvent('esx_dadgostari_job:OutVehicle')
AddEventHandler('esx_dadgostari_job:OutVehicle', function(ped)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local pedcoords = GetEntityCoords(GetPlayerPed(ped))
	if GetDistanceBetweenCoords(coords, pedcoords, true) < 5 then
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.dadgostariStations) do
...
Citizen.CreateThread(function()
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'dadgostari' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'dadgostari' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_dadgostari_job:hasExitedMarker', LastStation, LastPart, LastPartNum)
					hasExited = true
				end
				...
				TriggerEvent('esx_dadgostari_job:hasEnteredMarker', currentStation, currentPart, currentPartNum)
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'dadgostari' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicssleInGarage', 'dadgostari', vehicleProps)
					end
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'dadgostari' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'dadgostari_actions') then
			if Config.MaxInService == -1 then
...
RegisterNetEvent('esx_dadgostari_job:updateBlip')
AddEventHandler('esx_dadgostari_job:updateBlip', function()
	...
	if PlayerData.job ~= nil and PlayerData.job.name == 'dadgostari' then
		ESX.TriggerServerCallback('esx_society:getOnlissnePlayers', function(players)
			for i=1, #players, 1 do
				if players[i].job.name == 'dadgostari' then
...
AddEventHandler('playerSpawned', function(spawn)
	isDead = false
	TriggerEvent('esx_dadgostari_job:unrestrain')
	
	if not hasAlreadyJoined then
		TriggerServerEvent('esx_dadgostari_job:spawned')
	end
...
AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		TriggerEvent('esx_dadgostari_job:unrestrain')
		TriggerEvent('esx_phone:removeSpecialContact', 'dadgostari')

		if Config.MaxInService ~= -1 then
			TriggerServerEvent('esx_service:disableService', 'dadgostari')
		end

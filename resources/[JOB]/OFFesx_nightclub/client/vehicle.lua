...
local coorddx = -247.91
local coorddy = -228.75
local coorddz = 36.52
...
function Markerssss()
    while true do
        ...
        if GetDistanceBetweenCoords( coorddx,  coorddy, coorddz, GetEntityCoords(GetPlayerPed(-1)), false) < 10.0 then
            ...
            if ESX.GetPlayerData().job.name == 'nightclub' then
    ...
    if alirezaat() then
        if control == 'e' then
            if ESX.GetPlayerData().job.name == 'nightclub' then
                ...
                ESX.ShowNotification('shoma nightclub nistid')
            end
        end
    end
end)
...
function Spawning(model)
	...
	ESX.Game.SpawnVehicle(model, coords, 86.22, function(vehicle)
